...
import cloud.mallne.geokit.geojson.serialization.BoundingBoxSerializer
...
class BoundingBox internal constructor(internal val coordinates: DoubleArray) :
    Iterable<Double> {
    init {
     ...
     */
    operator fun get(index: Int): Double = coordinates[index]

    /** @return the coordinate at the given index or null if the index is out of range. */
    fun getOrNull(index: Int): Double? = coordinates.getOrNull(index)
     ...
     */
    fun toJson(): String = GeoJson.jsonFormat.encodeToString(this)
    ...
    companion object {
         ...
         */
        @JvmStatic
        fun fromJson(@Language("json") json: String): BoundingBox =
         ...
         */
        @JvmStatic
        fun fromJsonOrNull(@Language("json") json: String): BoundingBox? =
            try {
                GeoJson.jsonFormat.decodeFromString(json)
